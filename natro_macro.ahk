...
nm_SendHiveData(dataType, message, additionalData := {}) {
    ...
    try {
        ...
        if (IsObject(additionalData)) {
            try {
                for key, value in additionalData.OwnProps() {
                    ; Add all data - objects and simple values
                    requestData[key] := value
                }
            } catch {
                ; Continue without additional data if enumeration fails
            }
        }
        ...
        try {
            ...
            ; Fallback to simple JSON if stringify fails - LOG THE ERROR
            try {
...
MainGui.Add("CheckBox", "x20 y50 vWebTrackerEnabled Checked" WebTrackerEnabled, "Enable Web Tracking").OnEvent("Click", nm_HiveToggle)

; API Configuration
MainGui.Add("Text", "x20 y75 w100", "API Configuration:")
MainGui.Add("Text", "x20 y95", "API Key:")
MainGui.Add("Edit", "x80 y92 w300 h20 vWebTrackerToken", WebTrackerToken).OnEvent("Change", nm_HiveSaveConfig)
...
nm_HiveToggle(*) {
    ...
    WebTrackerEnabled := MainGui["WebTrackerEnabled"].Value
    IniWrite WebTrackerEnabled, "settings\nm_config.ini", "Settings", "WebTrackerEnabled"
...
nm_HiveSaveConfig(*) {
    global WebTrackerToken
    WebTrackerToken := MainGui["WebTrackerToken"].Value
    IniWrite WebTrackerToken, "settings\nm_config.ini", "Settings", "WebTrackerToken"
